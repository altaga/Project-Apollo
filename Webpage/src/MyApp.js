...
class MyApp extends Component {
  ...
  async checkDisplay2(addr) {
    ...
    await unirest('GET', 'https://rrplz186pb.execute-api.us-east-1.amazonaws.com/getData')
      ...
      await unirest('GET', 'https://rrplz186pb.execute-api.us-east-1.amazonaws.com/get-medicine')
      .headers({
        'address': addr
      })
      .end(function (res) {
        if (res.error) throw new Error(res.error);
        _this.state.medData = JSON.parse(res.raw_body)[0]
      });
  }
              ...
              <Card style={{ backgroundColor: "#282d2b" }}>
                <CardBody>
                  <div>
                    <Form>
                      <FormGroup>
                        <Label for="exampleSelect">Select Camera</Label>
                        <Input onChange={this.camSelect} type="select" name="select" id="cameraSelect">
                          {
